(function () {
  ...
  function seed() {
    ...
    return {
      ...
      restaurants: [{
        id: uid(), slug: "catalis-cafe", name: "Catalis Cafe", owner: "Mayur Jadhav",
        phone: "9999999999", city: "Pune", ownerPin: "1234", active: true, qrEnabled: true,
        plan: "Monthly", subscriptionEnds: now + days(30), paymentQr: DEFAULT_QR,
        upiId: "mayur@upi", upiName: "Mr. Mayur Ravindra Jadhav",
  ...
  function start() {
    ...
    window.addEventListener("hashchange", () => { customerCat = ""; render(); });
  }
  ...
  function mutate(fn) { const next = clone(state); fn(next); save(next); }
  ...
  function homeView() {
    ...
    return `
      ${topbar("home")}
      <main class="hero"><div class="hero-inner">
        <section>
          <h1>RestoQR Cloud</h1>
        ...
        <section class="demo-board">
          <div class="grid-2">
            ${stat("Restaurants",state.restaurants.length)}${stat("Active",active)}
            ${stat("Payment Checks",waiting)}${stat("Orders Today",today.length)}
          </div>
          <div class="card">
            <div class="section-head"><div><h2>Restaurant Operations</h2><p>Manage registrations, QR ordering, kitchen flow, billing, and customer reviews from one place.</p></div><span class="pill ok">Ready</span></div>
            <div class="row"><span class="muted">Restaurant owner access</span><a class="btn blue" href="#/owner">Owner Login</a></div>
          </div>
        </section>
      </div></main>`;
  }
  ...
  function adminView() {
    ...
    return `
      ...
      <main class="wrap">
        <div class="grid-4">
          ${stat("Restaurants",state.restaurants.length)}${stat("Active",state.restaurants.filter(r=>r.active).length)}
          ${stat("Orders",totalOrders)}${stat("Paid Value",money(revenue))}
        </div>
  ...
  function restaurantAdminCard(r) {
    ...
    return `
      <div class="list-item">
        <div class="row">
          <div><h3 style="margin:0">${esc(r.name)}</h3><p class="muted small" style="margin:4px 0 0">${esc(r.owner)} · ${esc(r.phone)} · /${esc(r.slug)}</p></div>
          <div class="row-left">
  ...
  function kitchenPanel(r) {
    ...
    return `
      <section class="card">
        <div class="section-head"><div><h2>Kitchen</h2><p>Paid orders appear here after counter verifies payment.</p></div></div>
        ${orders.map(o => orderCard(o, r)).join("") || empty("No paid kitchen orders")}
      </section>
      ${feedbackPanel(r)}`;
  }
  ...
  function billingPanel(r) {
    ...
    return `
      <section class="card">
        <div class="section-head"><div><h2>Billing Counter</h2><p>Check PhonePe payment, mark paid, and close tables.</p></div></div>
        ${orders.map(billCard).join("") || empty("No active bills")}
      </section>
      ${feedbackPanel(r)}`;
  }

  /* ─── Feedback panel shown on kitchen & billing ─── */
  function feedbackPanel(r) {
    const feedbacks = state.orders
      .filter(o => o.restaurantSlug === r.slug && o.feedback)
      .sort((a, b) => (b.feedbackAt || 0) - (a.feedbackAt || 0))
      .slice(0, 20);
    const stars = n => "★".repeat(n) + "☆".repeat(5 - n);
    return `
      <section class="card" style="margin-top:14px">
        <div class="section-head"><div><h2>Customer Feedback</h2><p>${feedbacks.length} review${feedbacks.length !== 1 ? "s" : ""} received</p></div></div>
        ${feedbacks.length ? feedbacks.map(o => `
          <div class="list-item">
            <div class="row">
              <div>
                <strong style="color:${o.feedbackRating >= 4 ? "var(--ok,#16a34a)" : o.feedbackRating >= 3 ? "var(--warn,#d97706)" : "var(--bad,#dc2626)"}">
                  ${stars(o.feedbackRating || 0)}
                </strong>
                <p class="muted small">Table ${o.table} · Order #${o.id.slice(-5).toUpperCase()} · ${o.feedbackAt ? new Date(o.feedbackAt).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"}) : ""}</p>
              </div>
              <span class="pill ${o.feedbackRating >= 4 ? "ok" : o.feedbackRating >= 3 ? "warn" : "bad"}">${o.feedbackRating}/5</span>
            </div>
            ${o.feedback ? `<p style="margin:6px 0 0;font-size:14px;font-style:italic">"${esc(o.feedback)}"</p>` : ""}
          </div>`).join("") : empty("No feedback yet — it will appear here once customers submit")}
      </section>`;
  }
  ...
  function customerView(params) {
    ...
    if (!isRestaurantOpen(r)) return customerShell(r.name,`<div class="empty">Ordering is currently unavailable. Please contact the counter.</div>`);
    ...
    if (lastOrder && lastOrder.status !== "completed") return customerShell(r.name, customerOrderTrackingView(r, lastOrder));
              ...
              ${r.addons.filter(a => a.active).map(a => `
                <div class="customer-item">
                  <label style="display:flex;align-items:center;gap:10px;cursor:pointer;flex:1">
                    <input type="checkbox" data-action="addon-toggle" data-addon="${a.id}" data-price="${a.price}" class="addon-chk" ${selectedAddons[a.id]?"checked":""} style="width:18px;height:18px;accent-color:var(--ok,#16a34a)">
  ...
  function customerItem(r, i) {
    ...
    return `<div class="customer-item">
      ...
      ${q
        ? `<div class="qty"><button data-action="cart-dec" data-id="${i.id}">-</button><strong>${q}</strong><button class="plus" data-action="cart-inc" data-id="${i.id}">+</button></div>`
        : `<button class="btn primary" data-action="cart-inc" data-id="${i.id}">Add</button>`}
    </div>`;
  }
  ...
  function checkoutBox(r, total) {
    ...
    const addonTotal = Object.keys(selectedAddons).reduce((s, id) => {
      const a = find(r.addons, id); return s + (a ? a.price : 0);
    }, 0);
    ...
    return `
      <style>
        ...
        .table-error-banner {
          background:#fef2f2;border:1.5px solid #ef4444;border-radius:10px;padding:10px 14px;
          display:flex;align-items:center;gap:10px;margin:0 0 10px;animation:rqShake .4s ease;
        }
        .table-error-banner span { color:#dc2626;font-weight:600;font-size:14px; }
      </style>
      <div class="cart-bar" style="display:flex;flex-direction:column;max-height:70vh;">
        <div id="table-error-slot"></div>
        <div class="cart-scrollable" style="overflow-y:auto;flex:1;min-height:0;padding-bottom:4px">
          ...
          <p class="muted small" style="margin-bottom:8px">Pay <strong>${money(grand)}</strong> to <strong>${esc(r.upiName||r.owner)}</strong></p>
  ...
  /* ─── Order tracking view (after order placed) ─── */
  function customerOrderTrackingView(r, o) {
    ...
    /* feedback form — only show once, not if already submitted */
    const feedbackSection = o.feedback ? `
      <div style="background:#f0fdf4;border:1.5px solid #16a34a;border-radius:12px;padding:14px;margin-bottom:12px;text-align:center">
        <p style="margin:0;font-weight:600;color:#15803d">✓ Thank you for your feedback!</p>
        <p style="margin:4px 0 0;font-size:13px;color:#166534">${"★".repeat(o.feedbackRating||0)}</p>
      </div>` : `
      <div style="background:var(--card,#fff);border:1px solid var(--line,#e5e7eb);border-radius:12px;padding:14px;margin-bottom:12px">
        <p style="margin:0 0 10px;font-weight:600;font-size:14px">💬 Share your feedback</p>
        <div style="display:flex;gap:6px;margin-bottom:10px" id="star-row">
          ${[1,2,3,4,5].map(n => `
            <button data-action="star-select" data-star="${n}" data-oid="${o.id}"
              style="flex:1;padding:8px 0;border:1.5px solid #d1d5db;border-radius:8px;background:#fff;font-size:20px;cursor:pointer;transition:background .15s">
              ☆
            </button>`).join("")}
        </div>
        <textarea id="feedback-text" placeholder="Tell us more (optional)..." rows="2"
          style="width:100%;box-sizing:border-box;margin-bottom:8px;border-radius:8px;border:1px solid #d1d5db;padding:8px;font-size:14px"></textarea>
        <button class="btn primary block" data-action="submit-feedback" data-oid="${o.id}" data-slug="${r.slug}">
          Submit Feedback
        </button>
      </div>`;
    ...
    return `
      <div style="padding:16px 14px">
        ...
        ${r.googleReviewUrl ? `
        <a href="${esc(r.googleReviewUrl)}" target="_blank" rel="noopener"
           style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:13px;border-radius:10px;background:#4285f4;color:#fff;font-weight:600;font-size:15px;text-decoration:none;margin-bottom:10px;box-sizing:border-box">
          ⭐ Review us on Google
        </a>` : ""}
  ...
  function orderCard(o, r) {
    const next = o.status==="pending"?"preparing":o.status==="preparing"?"ready":"completed";
    return `<div class="list-item">
      <div class="row">
        <div><strong>Table ${o.table}</strong><p class="muted small">#${o.id.slice(-5).toUpperCase()} · ${new Date(o.createdAt).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}</p></div>
        <span class="pill blue">${title(o.status)}</span>
      </div>
      ${orderLines(o)}
      ${o.note?`<p class="pill warn">Note: ${esc(o.note)}</p>`:""}
      ${o.feedback?`<p class="pill ok" style="margin-top:6px">★${o.feedbackRating}/5 — "${esc(o.feedback.slice(0,60))}${o.feedback.length>60?"…":""}"</p>`:""}
      <div class="row" style="margin-top:10px">
        <strong>${money(o.total)}</strong>
        <button class="btn ok" data-action="advance-order" data-id="${o.id}" data-next="${next}">${next==="completed"?"Complete":"Mark "+title(next)}</button>
      </div>
    </div>`;
  }
  ...
  function billCard(o) {
    return `<div class="list-item">
      <div class="row">
        <div><strong>Table ${o.table}</strong><p class="muted small">Order #${o.id.slice(-5).toUpperCase()}</p></div>
        <span class="pill ${o.paymentStatus==="paid"?"ok":"warn"}">${o.paymentStatus==="paid"?"Paid":"Check Payment"}</span>
      </div>
      ${orderLines(o)}
      ${o.feedback?`<p class="pill ok" style="margin-top:6px">★${o.feedbackRating}/5 — "${esc(o.feedback.slice(0,60))}${o.feedback.length>60?"…":""}"</p>`:""}
      <div class="row" style="margin-top:10px">
  ...
  function orderLines(o) {
    return `
      <div style="margin-top:10px">
        ${(o.items||[]).map(i=>`<div class="row small"><span>${esc(i.name)} x ${i.qty}</span><span>${money(i.price*i.qty)}</span></div>`).join("")}
        ${(o.addons||[]).map(a=>`<div class="row small"><span>${esc(a.name)}</span><span>${money(a.price)}</span></div>`).join("")}
      </div>`;
  }
  ...
  function bindClicks(e) {
    /* Addon checkboxes — handle before data-action lookup */
    const addonEl = e.target.closest(".addon-chk");
    if (addonEl) {
      const id = addonEl.dataset.addon;
      if (addonEl.checked) selectedAddons[id] = true; else delete selectedAddons[id];
      const r = bySlug(route().params.get("resto") || (state.restaurants[0]?.slug || ""));
      if (r) {
        const bar = document.querySelector(".cart-bar");
        if (bar) bar.outerHTML = checkoutBox(r, cartTotal(r));
      }
      return;
    }

    /* Star rating buttons */
    const starBtn = e.target.closest("[data-action='star-select']");
    if (starBtn) {
      const chosen = Number(starBtn.dataset.star);
      document.querySelectorAll("#star-row button").forEach((btn, i) => {
        btn.textContent = i < chosen ? "★" : "☆";
        btn.style.background = i < chosen ? "#fef08a" : "#fff";
        btn.style.borderColor = i < chosen ? "#eab308" : "#d1d5db";
        btn.dataset.chosen = i < chosen ? "1" : "";
      });
      starBtn.closest("#star-row").dataset.rating = chosen;
      return;
    }
    ...
    if (action === "update-price")   return updateRestaurant(el.dataset.slug, r => { const f=find(r.menu,el.dataset.id); if(f) f.price=Number(val("price-"+el.dataset.id))||f.price; });
    ...
    if (action === "dismiss-review") return localStorage.removeItem("restoqr_last_order_"+el.dataset.slug), cart={}, selectedAddons={}, customerCat="", render();
    ...
    if (action === "submit-feedback") return submitFeedback(el.dataset.oid, el.dataset.slug);
  }

  /* Also catch checkbox change events */
  document.addEventListener("change", function(e) {
    if (e.target.classList.contains("addon-chk")) {
      const id = e.target.dataset.addon;
      if (e.target.checked) selectedAddons[id] = true; else delete selectedAddons[id];
    }
  });
  ...
  function adminLogin() {
    if (val("admin-pin") === state.meta.adminPin) {
      localStorage.setItem("restoqr_admin_unlocked","yes"); toast("Admin unlocked"); render();
    } else toast("Wrong passcode");
  }

  function registerRestaurant() {
    const name=val("reg-name"),owner=val("reg-owner"),phone=val("reg-phone"),city=val("reg-city"),
          pin=val("reg-pin"),upiId=val("reg-upiid"),upi=val("reg-upi"),review=val("reg-review");
    ...
    mutate(s => s.restaurants.push({
      ...
      plan:"Pending", subscriptionEnds:Date.now(), paymentQr:DEFAULT_QR, upiId:upiId||"",
      upiName:upi||owner, googleReviewUrl:review,
  ...
  function ownerLogin(slug) {
    ...
    if (r && val("owner-pin") === r.ownerPin) {
      ...
      toast("Owner panel opened"); render();
  ...
  function addMenuItem(slug) {
    ...
    updateRestaurant(slug, r => { if(!r.categories.includes(category)) r.categories.push(category); r.menu.push(item(name,category,price,veg,[])); });
  }

  function addCommonItem(slug, index) {
    const c = COMMON_ITEMS[index]; if(!c) return;
  ...
  function addTable(slug) {
    ...
    updateRestaurant(slug, r => {
      ...
      r.tables.push({no,seats}); r.tables.sort((a,b)=>a.no-b.no);
    });
  }

  function saveSettings(slug) {
    updateRestaurant(slug, r => {
      r.name=val("set-name")||r.name; r.owner=val("set-owner")||r.owner; r.phone=val("set-phone")||r.phone;
      r.upiName=val("set-upi")||r.upiName; r.upiId=val("set-upiid");
      r.googleReviewUrl=val("set-review")||""; r.paymentQr=val("set-qr")||DEFAULT_QR;
    });
  }

  /* Read helpers — always pull from live DOM at action time */
  function getTableNumber() { const el=document.getElementById("customer-table"); return el?Number(el.value.trim()):0; }
  function getOrderNote()   { const el=document.getElementById("order-note");    return el?el.value.trim():""; }
  function getCheckedAddons() {
    const c={};
    document.querySelectorAll(".addon-chk:checked").forEach(el => c[el.dataset.addon]=true);
    return c;
  }
  ...
  function placeOrder(slug) {
    ...
    if (!table) {
      /* ── Red banner in the cart bar, shake the table input ── */
      const slot = document.getElementById("table-error-slot");
      const tableInput = document.getElementById("customer-table");
      if (slot) {
        slot.innerHTML = `
          <div class="table-error-banner">
            <span style="font-size:20px">⚠️</span>
            <span>Please enter your table number before confirming!</span>
          </div>`;
      }
      if (tableInput) {
        tableInput.style.border = "2px solid #ef4444";
        tableInput.style.animation = "rqShake .4s ease";
        tableInput.focus();
        setTimeout(() => { tableInput.style.animation=""; tableInput.style.border=""; }, 800);
      }
      return;
    }

    const liveAddons   = getCheckedAddons();
    const note         = getOrderNote();
    const pickedAddons = Object.keys(liveAddons).map(id=>find(r.addons,id)).filter(Boolean);
    const items = Object.entries(cart).map(([id,qty]) => {
      const m=find(r.menu,id); if(!m) return null;
    ...
    mutate(s => s.orders.push({
      ...
      note, total, paymentStatus:"waiting", status:"payment_check", createdAt:Date.now()
    }));

    localStorage.setItem("restoqr_last_order_"+slug, orderId);
    cart={}; selectedAddons={}; customerCat="";
    toast("Order placed! Waiting for payment verification.");
    render();
  }

  function submitFeedback(orderId, slug) {
    const starRow = document.getElementById("star-row");
    const rating  = starRow ? Number(starRow.dataset.rating||0) : 0;
    const text    = (document.getElementById("feedback-text")?.value||"").trim();
    if (!rating) return toast("Please select a star rating first");
    updateOrder(orderId, o => {
      o.feedback       = text;
      o.feedbackRating = rating;
      o.feedbackAt     = Date.now();
    });
    toast("Thank you for your feedback!");
  }

  function updateRestaurant(slug, fn) {
    mutate(s => { const r=s.restaurants.find(x=>x.slug===slug); if(r) fn(r); });
  }

  function updateOrder(id, fn) {
    mutate(s => { const o=s.orders.find(x=>x.id===id); if(o) fn(o); });
  }
